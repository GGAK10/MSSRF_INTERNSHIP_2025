...
def create_map(roi):
    ...
    # Use your own labeled training points here
    # These are assumed to be defined globally
    training = image.sampleRegions(
        collection=Water_Bodies.merge(Vegetation).merge(Built_Up).merge(Sand).merge(FallowLand),
        properties=['Class'],
        scale=10
    )

    classifier = ee.Classifier.smileRandomForest( numberOfTrees=100,
    minLeafPopulation=5 ).train(
        features=training,
        classProperty='Class',
        inputProperties=bands.bandNames()
    )
    print("Total samples:", training.size().getInfo())
    print("Samples per class:")
    class_counts = training.aggregate_histogram('Class').getInfo()
    print(class_counts)
    ...
    m.addLayer(image, {'bands': ['B4', 'B3', 'B2'], 'min': 300, 'max': 3000}, 'Satellite')
    m.addLayer(ndvi, {'min': -1, 'max': 1, 'palette': ['blue', 'white', 'green']}, 'NDVI')
    m.addLayer(classified, {'min': 1, 'max': 5, 'palette': ['yellow', 'blue', 'red', 'green', 'orange']}, 'LULC')

    # Add legend
    legend_html = """
    <div style="position: fixed; bottom: 70px; left: 48px; width: 180px;
    ...
    ui_html = f"""
    <script>
    function send_geometry() {{
        let geojson = null;
        let leafletMap = window.{m.get_name()};
        ...
        console.log("Sending ROI:", geojson);
    ...
    function export_layers() {{
        alert('Exporting layers...');
        fetch('/export_layers', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{
                ndvi: 'NDVI_output.tif',
                classified: 'LULC_output.tif',
                satellite: 'Satellite_output.tif'
            }})
        }})
        .then(response => response.json())
    ...
    <div style="position: fixed; bottom: 15px; right: 10px; z-index: 1000;">
        <button onclick="send_geometry()" style="background-color:#0078A8;color:white;padding:10px;border:none;border-radius:5px;">Finalize ROI</button>
        <button onclick="export_layers()" style="background-color:#4CAF50;color:white;padding:10px;border:none;border-radius:5px;margin-left:5px;">Export Layers</button>
    </div>
    """
    m.get_root().html.add_child(folium.Element(legend_html + ui_html))
    return m

@app.route('/')
def index():
    return create_map(user_roi).to_html()
...
def set_roi():
    ...
    try:
        user_roi = ee.Geometry(data['geometry'])
...
def export_layers():
    data = request.get_json()
    threading.Thread(target=export_layers_task, args=(data,)).start()
    return jsonify({'message': 'Export started. Check console for updates.'})

def download_from_drive(prefix, gauth):
    ...
    for file in file_list:
        ...
        filename = f"{file['title']}"
        access_token = gauth.credentials.access_token
        headers = {'Authorization': f'Bearer {access_token}'}

        response = requests.get(download_url, headers=headers, stream=True)
        with open(filename, 'wb') as f, tqdm(desc=filename, total=file_size, unit='B', unit_scale=True) as bar:
...
def export_layers_task(data):
    try:
        ...
        for prefix in tasks.keys():
            if tasks[prefix].status()['state'] == 'COMPLETED':
                download_from_drive(prefix, gauth)
            else:
                print(f"❌ {prefix} export failed.")
    except Exception as e:
